...
const ChatBot = function () {
    ...
    const sampleConversationRunning = false;

    // a callback for after a chat entry has been added
    const addChatEntryCallback;
    ...
    function updateCommandDescription() {
        ...
        for (i = 0; i < descriptions.length; i++) {
            const pdesc = descriptions[i].replace(/(['"][^'"]+['"])/gi, '<span class="phraseHighlight">$1</span>');
            pdesc = pdesc.replace(/(\[[^\[\]]+\])/gi, '<span class="placeholderHighlight">$1</span>');
            //console.log(pdesc);
            const matches = pdesc.match(/<span class=['"]phraseHighlight["']>['"](.+?)['"]<\/span>/gi);
            //console.log(matches);
            if (matches != null) {
                //console.log(matches);
                for (j = 0; j < matches.length; j++) {
                    const cleanMatch = matches[j].replace(/<\/?span[^>]*>/gi,'');
        ...
        const datalist = $('#chatBotCommands');
        if (datalist.size() == 0) {
    ...
    function playConversation(state, pauseLength) {
        ...
        setTimeout(function() {
            ...
            if (state.start < state.currentInput.length) {
                ...
                const chclb = $('#chatBotConversationLoadingBar');
                if (chclb.size() == 0) {
                ...
                const left =  $(inputs).offset().left;
                const top = $(inputs).offset().top + $(inputs).outerHeight() - 3;
    ...
    return {
        Engines: {
            ...
            webknox: function (apiKey) {
                ...
                const contextId = Math.random() * 100000;
                ...
                return {
                    react: function (query) {
                        $.get('https://webknox-question-answering.p.mashape.com/questions/converse?mashape-key=' + apiKey + '&contextId=' + contextId + '&text=' + encodeURIComponent(query), function (data) {
                            ...
                            const content = data.answerText;

                            if (data.media != undefined) {

                                content += '<br>';

                                for (const i = 0; i < data.media.length; i++) {
                                    const ob = data.media[i];
                ...
                return {
                    react: function (query) {
                        $.ajax({
                            ...
                            const content = data.AbstractText;
                            ...
                            if (content == '' && data.RelatedTopics.length > 0) {
                                ...
                                const media = [];
                                for (const i = 0; i < data.RelatedTopics.length; i++) {
                                    const ob = data.RelatedTopics[i];
        ...
        addChatEntry: function addChatEntry(text, origin) {
            ...
            const entryDiv = $('<div class="chatBotChatEntry ' + origin + '"></div>');
        ...
        thinking: function (on) {
            const ti = $('#chatBotThinkingIndicator');
            if (on) {
        ...
        react: function react(text) {
            ...
            for (const i = 0; i < patterns.length; i++) {
                ...
                if (matches) {
                    switch (pattern.actionKey) {
                        case 'rewrite':
                            ...
                            for (const j = 1; j < matches.length; j++) {
                            ...
                            break;
                        case 'response':
//                                const response = text.replace(r, pattern.actionValue);
                            const response = pattern.actionValue;
            ...
            for (const e = 0; e < engines.length; e++) {
